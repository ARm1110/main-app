...
<div class=" overflow-x-auto shadow-md sm:rounded-lg w-full">
    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 ">
       <a href="{{route('admin.childcategory.create')}}">افزودن جدید</a>
    </button>
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        @foreach($childCategories as $childcategory)
            ...
            <td class="px-6 py-4 text-right">
                <a href="{{ route('admin.childcategory.edit',['childcategory'=>$childcategory->id]) }}" class="font-medium text-blue-600 hover:underline">ویرایش</a>
                <form action="{{route('admin.childcategory.destroy',['childcategory'=>$childcategory->id])}}" method="POST" class="inline" >
                @csrf
                @method('DELETE')
